...
GLvoid ReSizeGLScene(GLsizei width, GLsizei height) {
    ...
    gluPerspective(120.0f, 0.88f, 0.1f, 1000.0f);
...
int InitGl(GLvoid) {
    ...
    floorimage = LoadTexture("floor.bmp", 255);
	whitetiles = LoadTexture("whitetiles.bmp",255); 
...
int drawCube(GLvoid) {
    ...
    // right
    glBindTexture(GL_TEXTURE_2D, RightBuilding);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 0);  
        glVertex3f(1.5f, -1.5f, -1.5f);  
        glTexCoord2f(1, 0);
	...
	glPushMatrix();
	glTranslatef(-2.0f,-3.0f,-10.0);
	//top 2 
	glBindTexture(GL_TEXTURE_2D, whitetiles);
	float f = 2.5 ; 
    glBegin(GL_QUADS);
		glNormal3f(0,1,0);
        glTexCoord2f(0, 0);  
	...
	int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Example", 1600,900, 16, fullscreen))
	{
